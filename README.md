# ✅Sistema de Gestión y Trazabilidad de Inventarios
Proyecto de Ingeniería – UNAD
Estado del proyecto: Diseño del prototipo (Figma)

Este repositorio contiene los avances del proyecto “Sistema de gestión y trazabilidad de inventarios para tiendas de autoservicio”.
Actualmente, el proyecto se encuentra en la fase de diseño de la solución, donde se ha desarrollado un prototipo funcional usando Figma.


# Prototipo interactivo en Figma

- Puedes acceder al diseño aquí:
https://www.figma.com/proto/OWmhvOKUAdHHrJZVggbCqa/Tienda?node-id=1-6&starting-point-node-id=1%3A6

- Se recomienda Presionar la tecla Z y F11 para tener una mejor vista con figma

# 📘 Descripción del proyecto

El objetivo del sistema es permitir a las tiendas de autoservicio gestionar su inventario en tiempo real mediante:

- Registro de entradas y salidas
- Métricas rápidas e indicadores
- Alertas por bajo stock
- Reportes mensuales
- Configuración del sistema (usuarios, parámetros, tienda y notificaciones)

# 📂 Estructura actual del repositorio

/docs               → Documentos del proyecto (metodología, informes)

/figma              → Enlaces y capturas del prototipo

/frontend           → Código del frontend (por desarrollar)

backend             → Código del backend (por desarrollar)

README.md           → Documentación general del proyecto


----------------------
👥 Autores

Edy Santiago Rico Villalobos

María Alejandra Castillo Figueroa

---------------------
📌 Estado actual

✔ Prototipo Figma finalizado

⬜ Inicio de desarrollo en código (pendiente)

⬜ Implementación backend

⬜ Integración con base de datos

⬜ Pruebas y documentación final








